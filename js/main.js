...
addEventListener("DOMContentLoaded", async () => {
    ...
    input__search.addEventListener("input", async e => {
        ...
        const searchTerm = e.target.value.toLowerCase().trim(); // Obtener el valor del input y convertirlo a minusculas y sin espacios

        const filteredProfiles = info.filter(person => {
            return person.name_full.toLowerCase().includes(searchTerm); // Comparar si el nombre de la persona incluye el termino de busqueda
        });
